...
c = 3;
    ...
    function f1p2p1(debug)      % DDI distribution
        ...
        figure(11);set(figure(11),'name','Distribution of DDI (vestibular vs. visual)','unit','normalized','pos',[-0.55 0.1 0.53 0.6]); clf;
        
        for pp = 1:2
            
            % scatter
            h{1}(pp) = axes('pos',[0.1+0.5*(pp-1) 0.2 0.28 0.5]);hold on;axis square;
            plot(h{1}(pp),[0 1],[0 1],'-','color',[0.7 0.7 0.7],'linewidth',1.5); % diagonal line
            plot(h{1}(pp),DDI_spatial{pp}.tempoSig(:,1),DDI_spatial{pp}.tempoSig(:,2),'ko','markersize',6,'markerfacecolor','w');
            plot(h{1}(pp),DDI_spatial{pp}.vestiSig(:,1),DDI_spatial{pp}.vestiSig(:,2),'ko','markersize',6,'markerfacecolor','b');
            plot(h{1}(pp),DDI_spatial{pp}.visSig(:,1),DDI_spatial{pp}.visSig(:,2),'ko','markersize',6,'markerfacecolor','r');
            plot(h{1}(pp),DDI_spatial{pp}.bothSig(:,1),DDI_spatial{pp}.bothSig(:,2),'ko','markersize',6,'markerfacecolor','k');
            
            text(0.05,0.9,['n(vestiSig) = ',num2str(sum(select_spatialSig{pp}(:,1)))],'color','b');
            text(0.05,0.8,['n(visSig) = ',num2str(sum(select_spatialSig{pp}(:,2)))],'color','r');
            text(0.05,0.7,['n(bothSig) = ',num2str(sum(select_spatialSig{pp}(:,3)))],'color','k');
            xlabel('DDI (Vestibular)');ylabel('DDI (Visual)');set(h{1}(pp),'xlim',[0 1],'ylim',[0,1]);
            hold off;
            
            % vestibular (bar)
            h{2}(pp) = axes('pos',[0.1+0.5*(pp-1) 0.75 0.28 0.12]);hold on;
            
            [n_vesti{pp}(1,:), ~] = hist(DDI_spatial{pp}.vestiSig(:,1),xDDI);
            [n_vesti{pp}(2,:), ~] = hist(DDI_spatial{pp}.visSig(:,1),xDDI);
            [n_vesti{pp}(3,:), ~] = hist(DDI_spatial{pp}.bothSig(:,1),xDDI);
            
            medianVesti{pp}(1) = median(DDI_spatial{pp}.vestiSig(:,1));
            medianVesti{pp}(2) = median(DDI_spatial{pp}.visSig(:,1));
            medianVesti{pp}(3) = median(DDI_spatial{pp}.bothSig(:,1));
            
            hbar{1}{pp} = bar(xDDI,n_vesti{pp}');
            set(hbar{1}{pp}(1),'facecolor','b','edgecolor','b');set(hbar{1}{pp}(2),'facecolor','r','edgecolor','r');set(hbar{1}{pp}(3),'facecolor','k','edgecolor','k');
            %             set(gca,'xtick',[]);
            ylabel('cell #');set(gca,'xlim',[0 1]);
            plot([medianVesti{pp}(1) medianVesti{pp}(1)],[0 max(n_vesti{pp}(:))*1.1],'b--','linewidth',1.5);
            text(medianVesti{pp}(1),max(n_vesti{pp}(:))*1.2,num2str(medianVesti{pp}(1)),'color','b','fontsize',4);
            plot([medianVesti{pp}(2) medianVesti{pp}(2)],[0 max(n_vesti{pp}(:))*1.1],'r--','linewidth',1.5);
            text(medianVesti{pp}(2),max(n_vesti{pp}(:))*1.2,num2str(medianVesti{pp}(2)),'color','r','fontsize',4);
            plot([medianVesti{pp}(3) medianVesti{pp}(3)],[0 max(n_vesti{pp}(:))*1.1],'k--','linewidth',1.5);
            text(medianVesti{pp}(3),max(n_vesti{pp}(:))*1.2,num2str(medianVesti{pp}(3)),'color','k','fontsize',4);
            hold off;
            
            
            % visual (bar)
            h{3}(pp) = axes('pos',[0.4+0.5*(pp-1) 0.23 0.08 0.45]);hold on;
            
            [n_vis{pp}(1,:), ~] = hist(DDI_spatial{pp}.vestiSig(:,2),xDDI);
            [n_vis{pp}(2,:), ~] = hist(DDI_spatial{pp}.visSig(:,2),xDDI);
            [n_vis{pp}(3,:), ~] = hist(DDI_spatial{pp}.bothSig(:,2),xDDI);
            
            medianVis{pp}(1) = median(DDI_spatial{pp}.vestiSig(:,2));
            medianVis{pp}(2) = median(DDI_spatial{pp}.visSig(:,2));
            medianVis{pp}(3) = median(DDI_spatial{pp}.bothSig(:,2));
            
            hbar{2}{pp} = bar(xDDI,n_vis{pp}');
            set(hbar{2}{pp}(1),'facecolor','b','edgecolor','b');set(hbar{2}{pp}(2),'facecolor','r','edgecolor','r');set(hbar{2}{pp}(3),'facecolor','k','edgecolor','k');
            %             set(gca,'xtick',[]);
            ylabel('cell #');set(gca,'xlim',[0 1]);
            plot([medianVis{pp}(1) medianVis{pp}(1)],[0 max(n_vis{pp}(:))*1.1],'b--','linewidth',1.5);
            text(medianVis{pp}(1),max(n_vis{pp}(:))*1.2,num2str(medianVis{pp}(1)),'color','b','fontsize',4);
            plot([medianVis{pp}(2) medianVis{pp}(2)],[0 max(n_vis{pp}(:))*1.1],'r--','linewidth',1.5);
            text(medianVis{pp}(2),max(n_vis{pp}(:))*1.2,num2str(medianVis{pp}(2)),'color','r','fontsize',4);
            plot([medianVis{pp}(3) medianVis{pp}(3)],[0 max(n_vis{pp}(:))*1.1],'k--','linewidth',1.5);
            text(medianVis{pp}(3),max(n_vis{pp}(:))*1.2,num2str(medianVis{pp}(3)),'color','k','fontsize',4);
            view(90,270);
            hold off;
            
        end
        
        suptitle(['Distribution of DDI   (Monkey = ',monkey_to_print,')']);
        SetFigure(12);
    ...
    function f1p2p2(debug)      % Preferred direction distribution
        ...
        figure(11);set(figure(11),'name','Distribution of preferred direction','unit','normalized','pos',[-0.55 0.25 0.53 0.6]); clf;
        ...
        for pp = 1:2
            % scatter
            h{1}(pp) = axes('pos',[0.1+0.5*(pp-1) 0.2 0.28 0.5]);hold on;axis square;
            plot(h{1}(pp),preDir_spatial{pp}.tempoSig{1}(:,1),preDir_spatial{pp}.tempoSig{1}(:,2),'bo','markersize',6,'markerfacecolor','w');
            plot(h{1}(pp),preDir_spatial{pp}.vestiSig{1}(:,1),preDir_spatial{pp}.vestiSig{1}(:,2),'ko','markersize',6,'markerfacecolor','b');
            
            plot(h{1}(pp),preDir_spatial{pp}.tempoSig{2}(:,1),preDir_spatial{pp}.tempoSig{2}(:,2),'ro','markersize',6,'markerfacecolor','w');
            plot(h{1}(pp),preDir_spatial{pp}.visSig{2}(:,1),preDir_spatial{pp}.visSig{2}(:,2),'ko','markersize',6,'markerfacecolor','r');
            
            %             xlabel('Preferred azimuth (degree)');ylabel('Preferred elevation (degree)');
            xlabel([{'\leftarrow        \uparrow        \rightarrow        \downarrow        \leftarrow'};{'Preferred azimuth (degree)'}],'fontsize',15);
            ylabel([{'Preferred elevation (degree)'};{'\leftarrow                  \downarrow                 \rightarrow'}],'fontsize',15);
            set(gca,'xtick',[0 90 180 270 360],'ytick',[-90 -45 0 45 90]);
            set(gca,'xticklabel',[],'yticklabel',[]);
            %             set(gca,'xticklabel',{'0','90','180','270','360'},'yticklabel',{'-90','-45','0','45','90'});
            set(h{1}(pp),'xlim',[0 360],'ylim',[-90 90]);set(gca,'ydir','reverse');
            hold off;
            
            % azimuth (bar)
            h{2}(pp) = axes('pos',[0.1+0.5*(pp-1) 0.75 0.28 0.12]);hold on;
            
            [n_azi{pp}(1,:), ~] = hist(preDir_spatial{pp}.vestiSig{1}(:,1),xazi);
            [n_azi{pp}(2,:), ~] = hist(preDir_spatial{pp}.visSig{2}(:,1),xazi);
            
            medianAzi{pp}(1) = median(preDir_spatial{pp}.vestiSig{1}(:,1));
            medianAzi{pp}(2) = median(preDir_spatial{pp}.visSig{2}(:,1));
            
            hbar{1}{pp} = bar(xazi,n_azi{pp}');
            set(hbar{1}{pp}(1),'facecolor','b','edgecolor','b');set(hbar{1}{pp}(2),'facecolor','r','edgecolor','r');
            set(gca,'xtick',[]);ylabel('cell #');set(gca,'xlim',[0 360]);
            plot([medianAzi{pp}(1) medianAzi{pp}(1)],[0 max(n_azi{pp}(:))*1.1],'b--','linewidth',1.5);
            text(medianAzi{pp}(1),max(n_azi{pp}(:))*1.2,num2str(medianAzi{pp}(1)),'color','b','fontsize',4);
            plot([medianAzi{pp}(2) medianAzi{pp}(2)],[0 max(n_azi{pp}(:))*1.1],'r--','linewidth',1.5);
            text(medianAzi{pp}(2),max(n_azi{pp}(:))*1.2,num2str(medianAzi{pp}(2)),'color','r','fontsize',4);
            hold off;
            
            % elevation (bar)
            h{3}(pp) = axes('pos',[0.4+0.5*(pp-1) 0.23 0.08 0.45]);hold on;
            
            [n_ele{pp}(1,:), ~] = hist(preDir_spatial{pp}.vestiSig{1}(:,2),xele);
            [n_ele{pp}(2,:), ~] = hist(preDir_spatial{pp}.visSig{2}(:,2),xele);
            
            medianEle{pp}(1) = median(preDir_spatial{pp}.vestiSig{1}(:,2));
            medianEle{pp}(2) = median(preDir_spatial{pp}.visSig{2}(:,2));
            
            
            hbar{2}{pp} = bar(xele,n_ele{pp}');
            set(hbar{2}{pp}(1),'facecolor','b','edgecolor','b');set(hbar{2}{pp}(2),'facecolor','r','edgecolor','r');
            set(gca,'xtick',[]);ylabel('cell #');set(gca,'xlim',[-90 90]);
            plot([medianEle{pp}(1) medianEle{pp}(1)],[0 max(n_ele{pp}(:))*1.1],'b--','linewidth',1.5);
            text(medianEle{pp}(1),max(n_ele{pp}(:))*1.2,num2str(medianEle{pp}(1)),'color','b','fontsize',4);
            plot([medianEle{pp}(2) medianEle{pp}(2)],[0 max(n_ele{pp}(:))*1.1],'r--','linewidth',1.5);
            text(medianEle{pp}(2),max(n_ele{pp}(:))*1.2,num2str(medianEle{pp}(2)),'color','r','fontsize',4);
            view(90,270);set(gca,'xdir','reverse');
            hold off;
            
            % text necessary infos
            axes('pos',[0.1+0.5*(pp-1) 0.05 0.28 0.05]);
            text(0,0.8,['n vestibular (vestiSig) = ',num2str(sum(select_spatialSig{pp}(:,1)))],'color','b');
            text(0,0,['n visual (visSig) = ',num2str(sum(select_spatialSig{pp}(:,2)))],'color','r');
            axis off;
            
        end
        
        suptitle(['Distribution of preferred direction   (Monkey = ',monkey_to_print,')']);
        SetFigure(12);
        
        % plot figures for delta preferred direction distribution (between vestibular & visual)
        
        xdiff = linspace(0,180,11);
        figure(12);set(figure(12),'name','Distribution of delta preferred direction (vestibular vs. visual)','unit','normalized','pos',[-0.55 -0.2 0.53 0.3]); clf;
    ...
    function f1p3p1(debug)      % DDI distribution between T & R
        ...
        % plot figures for DDI distribution (T DDI vs. R DDI)
        xDDI = linspace(0,1,11);
        figure(11);set(figure(11),'name','Distribution of DDI (T vs. R)','unit','normalized','pos',[-0.55 0.1 0.53 0.8]); clf;
        
        % scatter
        h{1}(pp) = axes('pos',[0.1 0.1 0.5 0.6]);hold on;axis square;
        plot(h{1}(pp),[0 1],[0 1],'-','color',[0.7 0.7 0.7],'linewidth',1.5); % diagonal line
        plot(h{1}(pp),DDI_spatial{1}.tempoSig(:,1),DDI_spatial{1}.tempoSig(:,2),'bo','markersize',8,'markerfacecolor','w');
        plot(h{1}(pp),DDI_spatial{2}.tempoSig(:,1),DDI_spatial{2}.tempoSig(:,2),'ro','markersize',8,'markerfacecolor','w');
        plot(h{1}(pp),DDI_spatial{1}.spatialSig(:,1),DDI_spatial{1}.spatialSig(:,2),'ko','markersize',8,'markerfacecolor','b');
        plot(h{1}(pp),DDI_spatial{2}.spatialSig(:,1),DDI_spatial{2}.spatialSig(:,2),'ko','markersize',8,'markerfacecolor','r');
        
        text(0.05,0.9,['n(vestiSig) = ',num2str(sum(select_spatialSig{1}(:,1)&select_spatialSig{2}(:,1)))],'color','b');
        text(0.05,0.8,['n(visSig) = ',num2str(sum(select_spatialSig{1}(:,2)&select_spatialSig{2}(:,2)))],'color','r');
        
        xlabel('DDI (Translation)');ylabel('DDI (Rotationl)');set(h{1}(pp),'xlim',[0 1],'ylim',[0,1]);
        hold off;
        
        % bar
        for jj = 1:2
            [n_T(jj,:), ~] = hist(DDI_spatial{jj}.spatialSig(:,1),xDDI);
            [n_R(jj,:), ~] = hist(DDI_spatial{jj}.spatialSig(:,2),xDDI);
            
            medianT{jj} = median(DDI_spatial{jj}.spatialSig(:,1));
            medianR{jj} = median(DDI_spatial{jj}.spatialSig(:,2));
        end
        
        % translation (bar)
        h{2} = axes('pos',[0.1 0.75 0.5 0.15]);hold on;
        
        hbar{1} = bar(xDDI,n_T');
        set(hbar{1}(1),'facecolor','b','edgecolor','b');set(hbar{1}(2),'facecolor','r','edgecolor','r');
        set(gca,'xtick',[]);ylabel('cell #');set(gca,'xlim',[0 1]);
        plot([medianT{1} medianT{1}],[0 max(n_T(:))*1.1],'b--','linewidth',1.5);
        text(medianT{1},max(n_T(:))*1.2,num2str(medianT{1}),'color','b','fontsize',4);
        plot([medianT{2} medianT{2}],[0 max(n_T(:))*1.1],'r--','linewidth',1.5);
        text(medianT{2},max(n_T(:))*1.2,num2str(medianT{2}),'color','r','fontsize',4);
        hold off;
        
        % rotation (bar)
        h{3} = axes('pos',[0.67 0.1 0.15 0.6]);hold on;
        
        hbar{2} = bar(xDDI,n_R');
        set(hbar{2}(1),'facecolor','b','edgecolor','b');set(hbar{2}(2),'facecolor','r','edgecolor','r');
        set(gca,'xtick',[]);ylabel('cell #');set(gca,'xlim',[0 1]);
        plot([medianR{1} medianR{1}],[0 max(n_R(:))*1.1],'b--','linewidth',1.5);
        text(medianR{1},max(n_R(:))*1.2,num2str(medianR{1}),'color','b','fontsize',4);
        plot([medianR{2}(1) medianR{2}],[0 max(n_R(:))*1.1],'r--','linewidth',1.5);
        text(medianR{2},max(n_R(:))*1.2,num2str(medianR{2}),'color','r','fontsize',4);
        view(90,270);
        hold off;
        
        suptitle(['Distribution of DDI (T & R)   (Monkey = ',monkey_to_print,')']);
        SetFigure(12);
    ...
    function f1p4p3(debug)      % DDI distribution between Fixation % Dark control
        ...
        plot(h{1}(pp),[0 1],[0 1],'-','color',[0.7 0.7 0.7],'linewidth',1.5); % diagonal line
        plot(h{1}(pp),DDI_spatial{1}.tempoSig,DDI_spatial{3}.tempoSig,'ko','markersize',8,'markerfacecolor','w');
        plot(h{1}(pp),DDI_spatial{2}.tempoSig,DDI_spatial{4}.tempoSig,'k^','markersize',8,'markerfacecolor','w');
        plot(h{1}(pp),DDI_spatial{1}.spatialSig,DDI_spatial{3}.spatialSig,'ko','markersize',8,'markerfacecolor','k');
        plot(h{1}(pp),DDI_spatial{2}.spatialSig,DDI_spatial{4}.spatialSig,'k^','markersize',8,'markerfacecolor','k');
        
        text(0.05,0.9,['n(Translation) = ',num2str(sum(select_spatialSig{3}(:,1)))]);
        text(0.05,0.8,['n(Rotation) = ',num2str(sum(select_spatialSig{4}(:,1)))]);
        
        xlabel('DDI (Vestibular)');ylabel('DDI (dark)');set(h{1}(pp),'xlim',[0 1],'ylim',[0,1]);
        hold off;
