function [filt_signal,Pl]=napl_band_pass(input, sampling_rate, lower_bound, upper_bound, tm_OR_fr,wind)
...
if (tm_OR_fr==1)
    input=fft(input,[]); % input=fft(input,[],2); changed by liang July 07 2011
end
our_wind = repmat(our_wind,1,size(input,2));
filt_signal=ifft(input.*our_wind,[],1,'symmetric');
max_power = repmat(max(abs(input)),floor(length(input)/2)+1,1);
Pl.wind = our_wind(1:floor(length(input)/2)+1,:).*max_power;
