function dpca_plot(pp,Xfull, W, V, plotFunction, varargin)

stimInd = 1:3;
title_text{1} = {'Left(blue)-right(red)','Up(blue)-down(red)','Forward(blue)-backward(red)'};
title_text{2} = {'pitch(nose up(blue)-down(red))','yaw(left(blue)-right(red))','roll(right(blue)-left(red))'};
% pp = 1; % translation
% pp = 2; % rotation
...
for ii = 1:length(stimInd)
    ...
    for c = 1:length(componentsToPlot)
        ...
        plotFunction(Zfull(cln{:}), options.time, [-thisYlim thisYlim], ...
            ...
            signifTrace, thisMarg,stimInd(ii))
