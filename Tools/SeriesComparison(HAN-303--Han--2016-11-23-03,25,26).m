        ...
        if sum(error_type == 3)>0 && ~all(errors == 0)
            for cc = 1:size(compare_index,2)
                ...
                if ~isempty(override_ps) % HH20160908
                    ...
                    if paras.Results.SEM  % Normal SEM and data
                    ...
                    else  % Bootstraping data. We should calculate the p-value using the definition. HH20161109
                        ps = [];
                        for tt = 1:size(y1,2)
                            ps(tt) = (0.5 - abs(rocN(y1(:,tt),y2(:,tt))-0.5))*2;  % This method should be rechecked!!!! 
                        end
                    end
                end
