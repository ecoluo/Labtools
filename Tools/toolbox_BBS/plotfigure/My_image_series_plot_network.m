function [ handle_1, handle_2] = My_image_series_plot_network(stimType,azi,anatomical_file,network_mask_file,adjust_method,Pvalue,cluster,mask_scaling,sliceon,sliceend,rows,columns,spacing)
...
threshold=T_Threshold(1);
neg_threshold=T_Threshold(2);
Mask_3D = permute(Mask_3D,[2 1 3]);
Mask_3D=Mask_3D(:,:,sliceon:sliceend);
Func_Img_3D = spmTimg.*double(Mask_3D);
% clustering
spmTimg_cluster = MY_clustering_function_map(Func_Img_3D,cluster);
...
network_image_series(network_image_series == min(network_image_series(:))) = 0;
[~, ~, handle_1, handle_2] = plot_network(stimType,azi,anatomical_file,anatomical_image_series,network_image_series,threshold,neg_threshold,mask_scaling);