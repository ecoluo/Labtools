function [sum_image ,network_color, handle_1, handle_2] = plot_network(stimType,azi,anatomical_file,anatomical,network_mask,threshold,neg_threshold,mask_scaling,invert_anatomical_colors,anatomical_scaling)
...
temp1 = strfind(anatomical_file,'.nii');
temp2 = strfind(anatomical_file,'_m');
tempN = anatomical_file(temp2+1:temp1-1);
if (nargout == 0) || (nargout >= 3)
    % Show the scale
    handle_1 = figure;imagesc(network_mask_orig);colorbar;colormap('jet');axis equal;axis off;title(['GLM correlation map: ',tempN,' ',stimType,' ',num2str(azi),'deg']);
    saveas(handle_1,['Z:\LBY\Recording data\fUS\GLMcorr_' ,tempN,'_',stimType,'_azi',num2str(azi)], 'jpg');
    % Show the image %modify by bobinshi
    %handle_2 = figure;imagesc(sum_image,[0 3]); colormap(network_color);axis equal;axis off;
    handle_2 = figure;imshow(sum_image,[0 3]); colormap(network_color);axis equal;axis off;title(['GLM correlation map with anatomical image: ',tempN,' ',stimType,' ',num2str(azi),'deg']);
    saveas(handle_2,['Z:\LBY\Recording data\fUS\GLMcorrAnatomy_' ,tempN,'_',stimType,'_azi',num2str(azi)], 'jpg');
end