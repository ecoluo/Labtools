...
%----------------------------------------------------------------------------------------------------------
function TEMPO_GUI_Switchyard_LA(action)
...
switch(action)
    ...
    case 'load data'
        ...
        if str2num(FILE(strfind(FILE,'m')+1:strfind(FILE,'c')-1)) == 6  % Qiaoqiao
            PATH = ['Z:\Data\MOOG\Qiaoqiao\raw\LA\',FILE(1:end-4),'\'];
        ...
        elseif str2num(FILE(strfind(FILE,'m')+1:strfind(FILE,'c')-1)) == 5  % Polo
            PATH = ['Z:\Data\MOOG\Polo\raw\LA\',FILE(1:end-4),'\'];
            set(PathHandle, 'String', PATH);
...
%             set(StopOffsetHandle, 'String', '-190');    % Delay from digMarker to visual onset. HH20140621
        ...
        [return_val, good_data, bad_data, newChans] = LoadTEMPOData_LA(PATH,FILE);
...
%             wavplay(audioread('Z:\labTools\Matlab\TEMPO_Analysis\type.wav'),200000,'async');
    ...
    case 'analyze'
        ...
        FileHandle = findobj(gcbf, 'Tag', 'Filename');
        FILE = get(FileHandle, 'String');
        FILE = FILE(1:end-4); % LBY added to delete .htb
