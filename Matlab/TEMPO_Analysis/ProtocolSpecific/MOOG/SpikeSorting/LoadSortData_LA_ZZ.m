...
% dfltDir = 'Z:\Data\LinearArray\Duncan\LAmerged\';% Default data directory
cd(dfltDir);

% Locate the batch file and check if anything was chosen.
% [dataPath] = uigetdir(uigetdir, 'Choose the direction of your files');
%  [dataPath] = uigetdir(dfltDir, 'Choose the direction of your files');
[dataFileName, dataPath] = uigetfile(dfltDir, 'Choose the direction of your files');
dataFileName = dataFileName(1:findstr(dataFileName,'.')-1);
if (dataFileName == 0)
    return;
end
...
if ~isempty(slashIndex)
    dataFileName = dataFileName(slashIndex(end)+1:end);
end

dataFileName=dataFileName(dataFileName~=' ');

OutFileName=['Z:\Data\MOOG\', monkeyName, '\Analysis\LASpikesorting\', dataFileName, '.mat'];
...
% mIndex = findstr(fileName,'m');
% dataPath = fileName(1:mIndex(2)-1);
slashmInd = findstr(fileName,'\m');
dataPath = fileName(1:slashmInd);
...
tempp = strfind(dataFileName, 'r');
% fid = fopen([dataFileName(1:tempp(2)-1),'\cluster_group.tsv']);
% fid = fopen([dataPath,'cluster_group.tsv']);
fid = fopen(['C:\Data\',dataFileName(strfind(dataFileName, 'm'):tempp(2)+1),'\cluster_group.tsv']);
...
spikeNumberInGUI = zeros(length(SelectNeuronID),1);
...
if isempty(findstr(fileName,'\'))    % If the directory has been ignored...
    ...
    switch monkeyN
        ...
        case 10
            handles.dataPath = 'Z:\Data\LinearArray\Messi\LAmerged\';
