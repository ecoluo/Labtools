...
% temp_mask_type = data.moog_params(MASK_TYPE,:,MOOG);

%now, remove trials from direction and spike_rates that do not fall between BegTrial and EndTrial
trials = 1:length(temp_azimuth);		% a vector of trial indices
% select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );
select_trials = trials ;
...
% mask_type=temp_mask_type(select_trials);
...
% unique_mask_type=munique(mask_type');
...
% one_repetition=length(unique_mask_type)*length(unique_heading)*length(unique_motion_coherence);
repetition=floor((EndTrial- BegTrial+1)/one_repetition);
EndTrial=one_repetition*repetition;
%now, remove trials from incomplete repetitions
select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );
stim_type = temp_stim_type( select_trials );
heading = temp_heading( select_trials );
amplitude= temp_amplitude( select_trials );
...
% if(length(unique_mask_type)>1)
%     stim_type=mask_type;
%     unique_stim_type=unique_mask_type;
% end
    ...
    for c = 1:length(unique_motion_coherence)
        ...
        for i = 1 : sum(unique_heading ~=0)   % subtract the 0 heading
            ...
            if i < (1+length(unique_heading))/2
                ...
                % compare to the 0 heading stim_type, which is straght ahead
                   Neuro_correct{c,k}(i) =  rocN(resp{c,k,(find(unique_heading==0))},resp{c,k,i},100 );
                   
                   
            else
                 % Neuro_correct{c,k}(i) =  rocN(resp{c,k,length(unique_heading)-i+ 1-sum(unique_heading == 0)}, resp{c,k,(i+sum(unique_heading==0))},100 ); 
                 % Neuro_correct{c,k}(i) = rocN(resp{c,k,i+1}, resp{c,k,i+1-(1+length(unique_heading))/2},100 );
                          % compare to the 0 heading stim_type, which is straght ahead
                  % Neuro_correct{c,k}(i) =  rocN(
                  % resp{c,k,(i+1)},resp{c,k,(find(unique_heading==0))},100
                  % );changed by YXF 20141212
                   Neuro_correct{c,k}(i) =  rocN( resp{c,k,(find(unique_heading==0))},resp{c,k,(i+1)},100 );
...
if abs(Bias_psy{1,1}) > 0.5
    [a,i_0]=min(abs(Bias_psy{1,1}-unique_heading)); % HH20130905
else
    i_0 = find(unique_heading >= 0,1);
end
i_0_orgin=find(unique_heading==0);%define 0 as 'dead-ahead' yxf 20141202
for k = 1:length(unique_stim_type)
    for c = 1:length(unique_motion_coherence)
        ...
        for n = 1 : perm_num
            ...
            resp_left_choose_perm = Z_Spikes_perm((heading == unique_heading(i_0)) & (stim_type == unique_stim_type(k)) & (motion_coherence == unique_motion_coherence(1)) & (choice == LEFT) & (Z_Spikes~=9999) );
            resp_right_choose_perm = Z_Spikes_perm((heading == unique_heading(i_0)) & (stim_type == unique_stim_type(k)) & (motion_coherence == unique_motion_coherence(1)) & (choice == RIGHT) & (Z_Spikes~=9999) );
            
            resp_left_choose_perm2 = Z_Spikes_perm((heading == unique_heading(i_0_orgin)) & (stim_type == unique_stim_type(k)) & (motion_coherence == unique_motion_coherence(1)) & (choice == LEFT) & (Z_Spikes~=9999) );
            resp_right_choose_perm2 = Z_Spikes_perm((heading == unique_heading(i_0_orgin)) & (stim_type == unique_stim_type(k)) & (motion_coherence == unique_motion_coherence(1)) & (choice == RIGHT) & (Z_Spikes~=9999) );
            %not zero heading ,but the nearest heading around bias;
            ...
            %Zero heading
            if  (length(resp_left_choose{c,k,i_0_orgin}) > 3) && (length(resp_right_choose{c,k,i_0_orgin}) > 3)
                CP_perm2(n) = rocN( resp_left_choose_perm2, resp_right_choose_perm2,100 );
            else
                CP_perm2(n) = NaN;
            end
            if  line_re{c,k} > 0
                CP_perm2(n) = 1 - CP_perm2(n);
            end
   ...
   % calculate p value for CP during zero heading
        
        if (length(resp_left_choose{c,k,i_0_orgin}) > 3) && (length(resp_right_choose{c,k,i_0_orgin}) > 3)
            hist_perm = [];
            hist_perm = hist( CP_perm2(:), x_bin );  % for permutation
            bin_sum = 0;
            n = 0;
            while ( n < (CP{c,k}(i_0_orgin)/bin) )
                n = n+1;
                bin_sum = bin_sum + hist_perm(n);
                if CP{c,k}(i_0_orgin) > 0.5    % note it's two tail test
                    pp_orgin = 2*(perm_num - bin_sum)/ perm_num;    % calculate p value for CP_all
                else
                    pp_orgin = 2* bin_sum / perm_num;
                end
            end
        ...
        p_a_orgin(c,k) = pp_orgin;
      
    end
end

%Temporally comment the figure to increase the speed,YXF,20180718
...
for k = 1:length(unique_stim_type)
    ...
    % Psychometric
    plot(unique_heading, psycho_correct{1}(k,:),m{k},'markerfacecolor',m{k}(1),'markersize',10);
    hold on;
...
% end
%%%%%Temporally comment to increase the batch speed,20180718,YXF
%{
...
for k = 1:length(unique_stim_type)
    ...
    ABC=[unique_heading,resp_mat{1,k}(:),resp_mat_err{1,k}(:)];
    xlabel('Heading Angle (deg)');
...
text(0,8, 'stim    Psy: u      threshold       Neu:u         threshold          CP\_all           p           CP\_0        p');

for k = 1:length(unique_stim_type)
    text(0,8-k*2, num2str(unique_stim_type(k)));
    text(5,8-k*2,sprintf('%5.3g       %5.3g',Bias_psy{1,k},Thresh_psy{1,k}));
    text(20,8-k*2,sprintf('%5.3g       %5.3g',Bias_neu{1,k},Thresh_neu{1,k}));
    text(40,8-k*2,sprintf('%5.3g       %5.3g',CP_all{1,k}',p{1,k}'));
    text(55,8-k*2,sprintf('%5.3g       %5.3g',CP{c,k}(i_0_orgin),p_a_orgin(1,k)));
    % Output to command line for Excel 
fprintf(' rep,	Psy_u,	Psy_thres,	Neu_u,	Neu_thres,	CP_all,	p_CP_all,	CP_0,	p_CP_0\n');
fprintf('%g\t %g\t %g\t %g\t %g\t %g\t %g\t %g\t %g\n', repetition, Bias_psy{1,k},Thresh_psy{1,k},Bias_neu{1,k},Thresh_neu{1,k},CP_all{1,k}',p{1,k}',CP{c,k}(i_0_orgin),p_a_orgin(1,k));
end
axis off;
...
if ~isempty(resp_all{c,k})
    ...
    title(sprintf('Grand \\itCP = \\rm%6.3g, \\itp = \\rm%4.3g, rep = %g',CP_all{c,k},p{c,k},repetition));
...
xbins = linspace(min([resp_left_choose{c,k,i_0}(:);resp_right_choose{c,k,i_0_orgin}(:)]),max([resp_left_choose{c,k,i_0_orgin}(:);resp_right_choose{c,k,i_0_orgin}(:)]),15);
if line_re{1} < 0  % Prefers left
    
    prefN = hist(resp_left_choose{c,k,i_0_orgin},xbins);
    nullN = hist(resp_right_choose{c,k,i_0_orgin},xbins);
else
    prefN = hist(resp_right_choose{c,k,i_0_orgin},xbins);
    nullN = hist(resp_left_choose{c,k,i_0_orgin},xbins);
end
...
ylabel('Frequency');
xlims = xlim();
title(sprintf('\\itCP \\rmat %g\\circ = \\rm%6.3g, \\itp = \\rm%4.3g, rep = %g ', unique_heading(i_0_orgin), CP{c,k}(i_0_orgin), p_a(c,k),repetition));
set(findall(gcf,'FontSize',10),'FontSize',13);
%}
...
%outfile = ['Z:\Xuefei_work\CP.dat'];
%outfile = [BASE_PATH 'ProtocolSpecific\MOOG\HeadingDiscrimination\HeadingDiscri_cum.dat'];
%outfile = ['Z:\Xuefei_work\Yuri\Heading_Discrim\MSTd_LocalTuning_SUMUCorr_MU.txt'];
%outfile = ['Z:\Xuefei_work\Yuri\Heading_Discrim\HeadingDisrim_MT_SUMUCorr_SMU.txt'];
%outfile = ['Z:\Xuefei_work\Yuri\Heading_Discrim\CP_4Microstim_MT_Yuri.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\CP_4Microstim_VIP.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\CP_SUMUCorr_SMU_VIP.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\Fine4Coarse_heading_CP_SUandMU_VIP.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\Fine4Coarse_heading_CP_SUandMU_VIP.txt'];
%buff = sprintf(sprint_txt, FILE,repetition, Bias_psy{1}, Thresh_psy{1}, Bias_neu{1}, Thresh_neu{1}, CP_all{1,1}, p{1,1},CP{1,1}(i_0_orgin), p_a_orgin(1,1),CP{1,1}(i_0), p_a(1,1), line_re{1,1}, line_p{1,1} );
%buff = sprintf(sprint_txt, FILE, Thresh_neu{1}, CP_all{1,1}, p{1,1},CP{1,1}(i_0_orgin), p_a_orgin(1,1));
%buff = sprintf(sprint_txt, FILE,CP_all{1,1},p{1,1},CP{1,1}(i_0_orgin),p_a_orgin(1,1));
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\LocalTuning_SUMUCorr_MU_MT.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\HeadingDiscrim_PsyPopulation.txt'];
%outfile = ['Z:\Xuefei_work\Summary_for_MicrostimCP\Heading_Repetition.txt'];
% buff_heading = sprintf(sprint_txt, FILE,fit_data_psycho_cum{1,1}(:,1),psy_perf{1,1});
buff_heading = sprintf(sprint_txt, FILE,repetition);
% buff_right_prop = sprintf(sprint_txt, FILE,fit_data_psycho_cum{1,1}(:,2),psy_perf{1,1});
% buff_total = sprintf(sprint_txt, FILE,fit_data_psycho_cum{1,1}(:,3),psy_perf{1,1});



%buff = sprintf(sprint_txt, FILE,Thresh_neu{1});
%buff = sprintf(sprint_txt, FILE,resp_mat{1,1}, line_re{1,1});
printflag = 0;
...
fid = fopen(outfile, 'a');
if (printflag)
    %fprintf(fid, 'FILE\t  repetition\tBias_psy\tThresh_psy\tBias_neu\tThresh_neu\tGrand_CP\tGrand_cp_p\tCP_0\tp_0\tCP_0\tp_0\t CP_new0\tp_new0\t k\tp_tuning\t');
    fprintf(fid, 'FILE\t  CP_all\t CP_all_p_value\t CP_0\t CP_0_p\t');
    fprintf(fid, '\r\n');
end
fprintf(fid, '%s', buff_heading);
fprintf(fid, '\r\n');
%{
fprintf(fid, '%s', buff_heading);
fprintf(fid, '\r\n');
fprintf(fid, '%s', buff_right_prop);
fprintf(fid, '\r\n');
fprintf(fid, '%s', buff_total);
fprintf(fid, '\r\n');
%}
fclose(fid);
%---------------------------------------------------------------------------------------
