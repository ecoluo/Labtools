...
function HeadingDis_cum_LBY(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, batch_flag);
...
for k = 1:length(unique_stim_type)
    ...
    progressbar(k/length(unique_stim_type),[]);
...
for k = 1:length(unique_stim_type)
    ...
    % Psychometric
    plot(unique_heading, psycho_correct{1}(k,:), h{k},'markerfacecolor',h{k}(1),'markersize',10);
    hold on;
    plot(xi, cum_gaussfit(psy_perf{1,k}, xi),  f{k},'linewidth',2.5);
    
    % Neurometric
    neu_heading = unique_heading(unique_heading~=0);
    plot(neu_heading, Neuro_correct{1,k}, '^','linewidth',2,'markersize',10,'color',h{k}(1));
    plot(xi, cum_gaussfit(neu_perf{1,k}, xi),  '--','linewidth',2.5,'color',h{k}(1));
    plot(xi, cum_gaussfit(neu_perf_anti{1,k}, xi),  ':','linewidth',1.5,'color',h{k}(1));
    ...
    % annotation
    text(max(unique_heading)/6,0.25-k*0.07,sprintf('%6.3g %6.3g(%6.3g)',psy_perf{1,k}(2),neu_perf{1,k}(2),neu_perf_anti{1,k}(2)),'color',h{k}(1));
end
...
for k = 1:length(unique_stim_type)
    ...
    xlim([min(unique_heading)*1.1,max(unique_heading)]*1.1);
    hold on;
    text(min(unique_heading),resp_mat{1,k}(1)*0.9,sprintf('%4.2g',line_re{k}),'color',h{k}(1));
end
title(['Tuning curve']);
set(gca,'xtickmode','auto');
