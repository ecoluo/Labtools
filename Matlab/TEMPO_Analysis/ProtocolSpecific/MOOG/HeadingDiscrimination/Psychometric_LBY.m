...
unique_heading = unique(temp_heading);
...
if length(unique_stimType) > 1 && length(unique_motion_coherence) > 1
...
if length(unique_motion_coherence)==1
    one_repetition = length(unique_heading)*length(unique_stimType);
else
    one_repetition = length(unique_heading)*length(unique_stimType)*length(unique_motion_coherence)-length(unique_heading);
end
...
for c = 1:length(unique_motion_coherence) % different coherence level
    for condition = 1:3
        if sum(unique_stimType == condition)==0 ;
...
if length(unique_motion_coherence)==1 && length(unique_stimType) ==3
...
for c = 1:length(unique_motion_coherence) % different coherence level
    for condition = 1:3
        if sum(unique_stimType == condition)==0 ;continue; end
        
        gaussfit = cum_gaussfit(psy_perf{c,condition}, xi);
        plot(unique_heading, fit_valid{c,condition}(:,2), symbo{c,condition},  xi, gaussfit,  fitline{c,condition} ,...
...
%%%%%%%%%%%%%%%%%%%%% Change here %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    result = PackResult(FILE, PATH, SpikeChan, repetitionN, unique_stimType, Protocol,... % Obligatory!!
                    unique_motion_coherence, choice_per_trial, unique_heading, ... % Trial info
                    Thresh_psy, Bias_psy, fit_valid, symbo, gaussfit, fitline);
...
switch Protocol
    case HEADING_DISCRIM % for heading discrimination task
        config.suffix = 'Psy_HD';
    case ROTATION_DISCRIM % for heading discrimination task
        config.suffix = 'Psy_RD';
end

config.xls_column_begin = 'Psy_n';
config.xls_column_end = 'Psy_vis_bias';

% Only once
config.sprint_once_marker = {'d','d'};
config.sprint_once_contents = 'result.repetitionN,result.unique_motion_coherence';


% Loop across each stim_type
config.sprint_loop_marker = {{'0.2f','0.2f'}};
config.sprint_loop_contents = {{'result.Thresh_psy{1,k},result.Bias_psy{1,k}'}};
