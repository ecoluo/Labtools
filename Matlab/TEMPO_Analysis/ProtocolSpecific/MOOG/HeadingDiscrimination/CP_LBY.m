function result = CP_LBY(headings,choices,spike_counts,CP_p_method, IF_FIGURE)
...
if ~if_fake
    ...
    for hh = 1:length(unique_heading)
        ...
        % (3) Dora tuning matrix (both heading and choice matter, put average firings into a heading-choice matrix,
        % like Dora's new manuscript) @HH20160907
        for cc = LEFT:RIGHT
            curr_heading_choice = (headings == unique_heading(hh)) & (choices == cc);
            Dora_tuning_matrix_n(cc,hh) = sum(curr_heading_choice);
            
            if Dora_tuning_matrix_n(cc,hh) > 0
                Dora_tuning_matrix_mean(cc,hh) = mean(spike_counts(curr_heading_choice));
                Dora_tuning_matrix_sem(cc,hh) = std(spike_counts(curr_heading_choice)) / sqrt(Dora_tuning_matrix_n(cc,hh));
            end
        end
    ...
    result.Neu_tuning_Dora_matrix_sem = Dora_tuning_matrix_sem;
...
for hh = 1:length(unique_heading)
    ...
    % @HH20160214. I put this part outside the "if" logic to get the spike counts
    % regardless of the number of choices, because I have to do the patch calucation for pref & null choices of 0
    % headings in GROUP_HD. (But I don't know when will I decide to rerun all the batch file...)
    
    % @HH20160906. HaHa, I finally decide to redo this! And also, I calculate the "Dora_tuning_matrix" today.
