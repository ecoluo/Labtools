...
function HeadingDis_cum_shiftwindow_LBY(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, batch_flag);
...
for k = 1:length(unique_stim_type)
    for c = 1:length(unique_motion_coherence)
        ...
        for i = 1:length(unique_heading)
            ...
            z_dist = (z_dist - mean(z_dist))/std(z_dist); % z = (x-mu)/sigma, standardization
...
% now calculate proportion correct from area under ROC curves, each heading is compared to 0 heading
% neurothreshold
fit_data_neuro = [];
fit_data_neuro_cut = [];
for k = 1 : length(unique_stim_type)
    for c = 1:length(unique_motion_coherence)
        ...
        for i = 1 : sum(unique_heading ~=0)   % subtract the 0 heading
            ...
            if i < (1+length(unique_heading))/2
                % in rocN, prefer first, but since plot rightward choice, thus reverse, here we assume leftward is the preferred direction
                ...
                if sum(unique_heading ==0) ==0      % If we don't have 0 heading
                    ...
                    Neuro_correct{c,k}(i) =  rocN( resp{c,k,(1+length(unique_heading))/2}, resp{c,k,(i+1)},100 ); % compare to the 0 heading stim_type, which is straight ahead
                end
...
set(11,'Position', [-1020,40, 750,750], 'Name', 'Psycho-neurometic function & CP', 'color','w');
orient landscape;
