...
pathname = {'Z:\Data\TEMPO\BATCH\MSTd_vis'};
        ...
        switch Protocol
            case {1 3}
                ...
                for ii = 1:length(filename)
                    ...
                    QQ_3DTuning_T(ii).monkey = str2double(filename(ii).name(strfind(filename(ii).name,'m')+1:strfind(filename(ii).name,'c')-1));
                    QQ_3DTuning_T(ii).name = str2double(filename(ii).name(strfind(filename(ii).name,'c')+1:strfind(filename(ii).name,'r')-1));
            ...
            case {2 4}
                ...
                for ii = 1:length(filename)
                    ...
                    QQ_3DTuning_R(ii).PSTH = temp.result.PSTH.spk_data_bin_mean_rate_aov;
