...
function [modelFitRespon_VAJ,modelFit_VAJ, modelFit_VAJ_spatial, modelFitPara_VAJ, BIC_VAJ, RSquared_VAJ, rss_VAJ, time] = fitVAJ_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VAJ_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VAJ_Model_1D', ...
...
respon = VAJ_Model_1D(modelFitPara_VAJ,st_data);
modelFitRespon_VAJ = respon;

modelFit_VAJ.V = VAJ_V_Com_1D(modelFitPara_VAJ([1:6,13,14]),st_data);
modelFit_VAJ.A = VAJ_A_Com_1D(modelFitPara_VAJ([1:3,7:9,13,14]),st_data);
modelFit_VAJ.J = VAJ_J_Com_1D(modelFitPara_VAJ([1:3,10:12,13,14]),st_data);

% model fit spatial tuning
modelFit_VAJ_spatial.V = cos_tuning_1D(modelFitPara_VAJ(4:6),st_data(1:13));
modelFit_VAJ_spatial.A = cos_tuning_1D(modelFitPara_VAJ(7:9),st_data(1:13));
modelFit_VAJ_spatial.J = cos_tuning_1D(modelFitPara_VAJ(10:12),st_data(1:13));
