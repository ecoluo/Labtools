...
function [modelFitRespon_JO, modelFit_JO, modelFit_JO_spatial, modelFitPara_JO, BIC_JO, RSquared_JO, rss_JO, time] = fitJO_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('JO_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('JO_Model_1D', ...
