...
function [modelFitRespon_VP,modelFit_VP, modelFit_VP_spatial, modelFitPara_VP, BIC_VP, RSquared_VP, rss_VP, time] = fitVP_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VP_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VP_Model_1D', ...
...
respon = VP_Model_1D(modelFitPara_VP,st_data);
modelFitRespon_VP = respon;

modelFit_VP.V = VP_V_Com_1D(modelFitPara_VP([1:6,10]),st_data);
modelFit_VP.P = VP_P_Com_1D(modelFitPara_VP([1:3,7:10]),st_data);
