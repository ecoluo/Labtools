...
function [modelFitRespon_PVAJ, modelFit_PVAJ, modelFit_PVAJ_spatial, modelFitPara_PVAJ, BIC_PVAJ, RSquared_PVAJ, rss_PVAJ, time] = fitPVAJ_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('PVAJ_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('PVAJ_Model_1D', ...
...
respon = PVAJ_Model_1D(modelFitPara_PVAJ,st_data);
modelFitRespon_PVAJ = respon;


modelFit_PVAJ.V = PVAJ_V_Com_1D(modelFitPara_PVAJ([1:6,16:18]),st_data);
modelFit_PVAJ.A = PVAJ_A_Com_1D(modelFitPara_PVAJ([1:3,7:9,16:18]),st_data);
modelFit_PVAJ.J = PVAJ_J_Com_1D(modelFitPara_PVAJ([1:3,10:12,16:18]),st_data);
modelFit_PVAJ.P = PVAJ_P_Com_1D(modelFitPara_PVAJ([1:3,13:15,16:18]),st_data);

% model fit spatial tuning
modelFit_PVAJ_spatial.V = cos_tuning_1D(modelFitPara_PVAJ(4:6),st_data(1:13));
modelFit_PVAJ_spatial.A = cos_tuning_1D(modelFitPara_PVAJ(7:9),st_data(1:13));
modelFit_PVAJ_spatial.J = cos_tuning_1D(modelFitPara_PVAJ(10:12),st_data(1:13));
modelFit_PVAJ_spatial.P = cos_tuning_1D(modelFitPara_PVAJ(13:15),st_data(1:13));
%% analysis
data_num = 26*nBins;
para_num = 18;
