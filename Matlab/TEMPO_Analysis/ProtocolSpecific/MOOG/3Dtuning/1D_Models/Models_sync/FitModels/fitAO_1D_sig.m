% fit Acceleration-only model for 1D tuning
% time (unit: s)
...
function [modelFitRespon_AO, modelFit_AO, modelFit_AO_spatial,modelFitPara_AO, BIC_AO, RSquared_AO, rss_AO, time, AO_peak_A_T] = fitAO_1D_sig(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration,sig)
...
gauss_time = acc_func([mu sig],time); % theoretical time profile
[corrcoeff_acc,lags] = xcorr(gauss_time, temporal_data,20, 'coeff');
neg_lags_idx = lags < 0;
neg_lags = lags(neg_lags_idx);
[~,max_val] = max(abs(corrcoeff_acc(neg_lags_idx)));
delay_i = neg_lags(max_val);
peak_i = find(time >= mu, 1, 'first')-1;
% find the initial mu
mu_0 = time(peak_i - delay_i);

if corrcoeff_acc(max_val) < 0
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('AO_Model_1D_sig', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('AO_Model_1D_sig', ...
...
respon = AO_Model_1D_sig(modelFitPara_AO,st_data);
