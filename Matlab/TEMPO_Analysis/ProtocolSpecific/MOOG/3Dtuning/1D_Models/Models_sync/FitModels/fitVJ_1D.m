...
function [modelFitRespon_VJ,modelFit_VJ, modelFit_VJ_spatial, modelFitPara_VJ, BIC_VJ, RSquared_VJ, rss_VJ, time] = fitVJ_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VJ_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VJ_Model_1D', ...
...
respon = VJ_Model_1D(modelFitPara_VJ,st_data);
modelFitRespon_VJ = respon;

modelFit_VJ.V = VJ_V_Com_1D(modelFitPara_VJ([1:6,10]),st_data);
modelFit_VA.J = VJ_J_Com_1D(modelFitPara_VJ([1:3,7:10]),st_data);
