...
function [modelFitRespon_VAP,modelFit_VAP, modelFit_VAP_spatial, modelFitPara_VAP, BIC_VAP, RSquared_VAP, rss_VAP, time] = fitVAP_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VAP_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VAP_Model_1D', ...
...
respon = VAP_Model_1D(modelFitPara_VAP,st_data);
modelFitRespon_VAP = respon;

modelFit_VAP.V = VAP_V_Com_1D(modelFitPara_VAP([1:6,13,14]),st_data);
modelFit_VAP.A = VAP_A_Com_1D(modelFitPara_VAP([1:3,7:9,13,14]),st_data);
modelFit_VAP.P = VAP_P_Com_1D(modelFitPara_VAP([1:3,10:12,13,14]),st_data);

% model fit spatial tuning
modelFit_VAP_spatial.V = cos_tuning_1D(modelFitPara_VAP(4:6),st_data(1:13));
modelFit_VAP_spatial.A = cos_tuning_1D(modelFitPara_VAP(7:9),st_data(1:13));
modelFit_VAP_spatial.P = cos_tuning_1D(modelFitPara_VAP(10:12),st_data(1:13));
