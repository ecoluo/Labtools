...
function [modelFitRespon_AJ,modelFit_AJ, modelFit_AJ_spatial, modelFitPara_AJ, BIC_AJ, RSquared_AJ, rss_AJ, time] = fitAJ_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('AJ_Model_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('AJ_Model_1D', ...
...
respon = AJ_Model_1D(modelFitPara_AJ,st_data);
modelFitRespon_AJ = respon;

modelFit_AJ.A = AJ_A_Com_1D(modelFitPara_AJ([1:6,10]),st_data);
modelFit_AJ.J = AJ_J_Com_1D(modelFitPara_AJ([1:3,7:10]),st_data);
