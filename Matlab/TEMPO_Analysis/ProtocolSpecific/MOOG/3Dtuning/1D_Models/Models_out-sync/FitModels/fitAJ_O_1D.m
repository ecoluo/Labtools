...
function [modelFitRespon_AJ,modelFit_AJ, modelFit_AJ_spatial, modelFitPara_AJ, BIC_AJ, RSquared_AJ, rss_AJ, time] = fitAJ_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)

sprintf('Fitting AJ 1D model...')
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('AJ_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('AJ_Model_O_1D', ...
...
respon = AJ_Model_O_1D(modelFitPara_AJ,st_data);
modelFitRespon_AJ = respon;

modelFit_AJ.A = AJ_A_Com_O_1D(modelFitPara_AJ([1:6,10]),st_data);
modelFit_AJ.J = AJ_J_Com_O_1D(modelFitPara_AJ([1:3,7:10,11]),st_data);

% model fit spatial tuning
modelFit_AJ_spatial.A = cos_tuning_1D(modelFitPara_AJ(4:6),st_data(1:8));
modelFit_AJ_spatial.J = cos_tuning_1D(modelFitPara_AJ(7:9),st_data(1:8));
%% analysis
data_num = 26*nBins;
para_num = 11;
