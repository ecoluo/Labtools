...
function [modelFitRespon_JO, modelFit_JO, modelFit_JO_spatial, modelFitPara_JO, BIC_JO, RSquared_JO, rss_JO, time] = fitJO_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)

sprintf('Fitting JO 1D model...')
...
gauss_time = jerk_func([mu sig],time); % theoretical time profile
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('JO_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('JO_Model_O_1D', ...
...
respon = JO_Model_1D(modelFitPara_JO,st_data);
