...
function [modelFitRespon_AO, modelFit_AO, modelFit_AO_spatial,modelFitPara_AO, BIC_AO, RSquared_AO, rss_AO, time] = fitAO_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('AO_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('AO_Model_O_1D', ...
...
respon = AO_Model_1D(modelFitPara_AO,st_data);
