...
function [modelFitRespon_PO, modelFit_PO, modelFit_PO_spatial,modelFitPara_PO, BIC_PO, RSquared_PO, rss_PO, time] = fitPO_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)

sprintf('Fitting PO 1D model...')
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('PO_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('PO_Model_O_1D', ...
...
respon = PO_Model_1D(modelFitPara_PO,st_data);
