...
function [modelFitRespon_VA,modelFit_VA, modelFit_VA_spatial, modelFitPara_VA, BIC_VA, RSquared_VA, rss_VA, time] = fitVA_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VA_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VA_Model_O_1D', ...
...
respon = VA_Model_O_1D(modelFitPara_VA,st_data);
modelFitRespon_VA = respon;

modelFit_VA.V = VA_V_Com_O_1D(modelFitPara_VA([1:6,10,11]),st_data);
modelFit_VA.A = VA_A_Com_O_1D(modelFitPara_VA([1:3,7:10]),st_data);
