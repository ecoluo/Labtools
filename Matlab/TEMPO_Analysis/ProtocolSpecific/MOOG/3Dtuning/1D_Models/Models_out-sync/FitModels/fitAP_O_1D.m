...
function [modelFitRespon_AP,modelFit_AP, modelFit_AP_spatial, modelFitPara_AP, BIC_AP, RSquared_AP, rss_AP, time] = fitAP_O_1D(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('AP_Model_O_1D', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('AP_Model_O_1D', ...
...
respon = AP_Model_O_1D(modelFitPara_AP,st_data);
modelFitRespon_AP = respon;

modelFit_AP.A = AP_A_Com_O_1D(modelFitPara_AP([1:6,10]),st_data);
modelFit_AP.P = AP_P_Com_O_1D(modelFitPara_AP([1:3,7:10,11]),st_data);
