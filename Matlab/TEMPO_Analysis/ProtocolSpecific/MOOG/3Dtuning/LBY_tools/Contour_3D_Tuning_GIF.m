...
for k = 1:length(unique_stimType)
    ...
    for ii = stimOnBin : stimOffBin
        ...
        %         if sum(sum(spk_data_bin_mean_rate_trans{k}(:,:,ii))) ~= 0
        ...
        set(gca, 'ytick', [] );
        axis off;hold on;
        if sum(sum(spk_data_bin_mean_rate_trans{k}(:,:,ii))) == 0 % for all zero in all directions
            patch([0 0 1 1],[0 1 1 0],[0,0,0.5625]);
            axis off;
        end
        %         elseif sum(sum(spk_data_bin_mean_rate_trans{k}(:,:,ii))) == 0
        %             patch([0 0 0.81 0.81],[0 1 1 0],[0,0,0.5625]);
        %             title(gca,['t = ',num2str((ii*timeStep-tOffset1)/1000),' s']);
        %             set(gca, 'xtick', [] );
        %             set(gca, 'ytick', [] );
        %             axis off;
        %         end
        % errobar of azimuth
        axes('unit','pixels','pos',[200 50 610 100]);
        ...
        axes('unit','pixels','pos',[90 162 100 395]);
        ...
        % text on the figure
        axes('unit','pixels','pos',[100 730 1800 80]);
        xlim([0,100]);
        ylim([0,10]);
        ...
        axes('unit','pixels','pos',[20 650 1800 100]);
        xlim([0,100]);
        ylim([0,10]);
        ...
        if Protocol == DIRECTION_TUNING_3D
            ss = [str, '_T'];
            file = ['/gpfsdata/home/byliu/Z/',PSTH.monkey,'/3D_Tuning/Translation/' ss,'.gif'];
