...
for i = 1:length(unique_orient)
    axes(h_subplot(i+9));
    errorbar(PSTH.opt_bin_rate{iAzi(i)},PSTH.opt_bin_mean_rate_ste(iAzi(i),:),'color','k');
    hold on;
    for n = 1:size(markers,1)
        plot([markers{n,3} markers{n,3}], [0,max(PSTH.opt_maxSpkRealBinMean,PSTH.maxSpkSponbin_mean)], '--','color',markers{n,4},'linewidth',3);
        hold on;
    end
    set(gca,'ylim',[0 max([PSTH.opt_maxSpkRealBinMean+PSTH.opt_maxSpkRealBinMeanSte,PSTH.maxSpkSponbin_mean+PSTH.maxSpkSponbin_meanSte, PSTH.maxSpkRealbin_mean+PSTH.maxSpkRealbin_meanSte])],'xlim',[1 nBins(1,1)]);
    %             axis off;
    SetFigure(15);
...
for ss = 1:length(unique_spatFre)
    for tt = 1:length(unique_tempFre)
        for i = 1:length(unique_orient)
            axes(h_subplot(i+(tt-1)*9+(ss-1)*18+18));
            errorbar(squeeze(PSTH.bin_mean_rate(iAzi(i),ss,tt,:)),squeeze(PSTH.bin_mean_rate_ste(iAzi(i),ss,tt,:)),'color','k');
            hold on;
            for n = 1:size(markers,1)
                plot([markers{n,3} markers{n,3}], [0,max(PSTH.maxSpkRealbin_mean,PSTH.maxSpkSponbin_mean)], '--','color',markers{n,4},'linewidth',3);
                hold on;
            end
            set(gca,'ylim',[0 max([PSTH.opt_maxSpkRealBinMean+PSTH.opt_maxSpkRealBinMeanSte,PSTH.maxSpkSponbin_mean+PSTH.maxSpkSponbin_meanSte, PSTH.maxSpkRealbin_mean+PSTH.maxSpkRealbin_meanSte])],'xlim',[1 nBins(1,1)]);
            %             axis off;
...
% text on the figure
axes('unit','pixels','pos',[60 910 1800 80]);
xlim([0,100]);
ylim([0,10]);
...
axes('unit','pixels','pos',[60 0 1800 100]);
xlim([0,100]);
ylim([0,10]);
