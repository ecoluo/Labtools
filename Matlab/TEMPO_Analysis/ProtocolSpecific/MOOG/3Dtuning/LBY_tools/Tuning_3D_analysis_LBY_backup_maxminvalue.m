...
for k = 1:length(unique_stimType)
    ...
    for pc = 1:size(PSTH.spk_data_bin_rate_aov{k},1)
        PSTH.maxSigBin{k}(pc) = nan; % find if this bin is sig.
        PSTH.minSigBin{k}(pc) = nan; % find if this bin is sig.
        PSTH.sigTrue{k}(pc) = 0; % PSTH.sigTrue(pc) -> sig. of this direction
        
        % find time bin of the max % min value
        [~,maxBin{k}] = max(PSTH.spk_data_bin_mean_rate_aov{k}(pc,stimOnBin+9:stimOffBin-2));maxBin{k} = maxBin{k}+9;
        [~,minBin{k}] = min(PSTH.spk_data_bin_mean_rate_aov{k}(pc,stimOnBin+9:stimOffBin-2));minBin{k} = minBin{k}+9;
        % if 5 consecutive bins are sig.,then we say this bin is sig.
        % That is, this direction is temporally responded
        for ii = maxBin{k}(1)-2:maxBin{k}(1)+2
            try
        ...
        if sPeak{k}(pc) == 5
            PSTH.maxSigBin{k}(pc)= maxBin{k}; %
        end
        for ii = minBin{k}(1)-2:minBin{k}(1)+2
            if ranksum(squeeze(PSTH.spk_data_bin_rate_aov{k}(pc,ii,:))',squeeze(nanmean(PSTH.spk_data_bin_rate_aov{k}(pc,baselineBinBeg:baselineBinEnd,:)))') < 0.01
                sTrough{k}(pc) =  sTrough{k}(pc)+1;
            end
        end
        if sTrough{k}(pc) == 5
            PSTH.minSigBin{k}(pc)= minBin{k};
        end
        
        
        if (~isnan(PSTH.maxSigBin{k}(pc))) || (~isnan(PSTH.minSigBin{k}(pc)))
    ...
    if PSTH.respon_sigTrue(k) == 1
        ...
        for pt = 1:PSTH.NoPeaks(k)
            for pc = 1:size(PSTH.spk_data_bin_rate_aov{k},1)
                resp_sse_t{k}(pc,pt) = sum((PSTH.spk_data_bin_rate_aov{k}(pc,PSTH.peak{k}(pt),:) - mean(PSTH.spk_data_bin_rate_aov{k}(pc,PSTH.peak{k}(pt),:))).^2); % for DDI
...
for k = 1:length(unique_stimType)
    ...
    if PSTH.respon_sigTrue(k) == 1
        ...
        switch spon_flag
            case 0 %raw data
                models_fitting(a,model_catg,models,models_color,FILE,SpikeChan, Protocol,k,meanSpon,PSTH.spk_data_bin_mean_rate{k}(:,:,stimOnBin:stimOffBin),PSTH.spk_data_count_mean_rate_all{k},PSTH.spon_spk_data_bin_mean_rate(stimOnBin:stimOffBin),nBins,reps,markers,stimOnBin,stimOffBin,aMax,aMin,timeStep,unique_duration);
