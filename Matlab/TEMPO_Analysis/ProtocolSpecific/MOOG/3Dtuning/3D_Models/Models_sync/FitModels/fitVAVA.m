% fit Velocity-acceleration model for 3D tuning 20190805
% add a velocity*acceleration component
% time (unit: s)
...
function [modelFitRespon_VAVA,modelFit_VAVA, modelFit_VAVA_spatial, modelFitPara_VAVA, BIC_VAVA, RSquared_VAVA, rss_VAVA, time] = fitVAVA(spon,PSTH_data,spatial_data, nBins,reps,stimOnBin,stimOffBin,aMax,aMin,duration)

% sprintf('Fitting VAVA model...')
...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VAVA_Model', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VAVA_Model', ...
...
modelFitPara_VAVA = rand_param(min_inx,:);
rss_VAVA = rand_rss(min_inx);
jac_VAVA = rand_jac(min_inx,:,:);

% calculate the final model fitting values
respon = VAVA_Model(modelFitPara_VAVA,st_data);
modelFitRespon_VAVA = respon;

modelFit_VAVA.V = VAVA_V_Com(modelFitPara_VAVA([1:7,12]),st_data);
modelFit_VAVA.A = VAVA_A_Com(modelFitPara_VAVA([1:3,8:12]),st_data);

% model fit spatial tuning
modelFit_VAVA_spatial.V = cos_tuning(modelFitPara_VAVA(4:7),st_data(1:13));
modelFit_VAVA_spatial.A = cos_tuning(modelFitPara_VAVA(8:11),st_data(1:13));

%% analysis
data_num = 26*nBins;
para_num = 12;
BIC_VAVA = BIC_fit(data_num,rss_VAVA,para_num);
TSS = sum((PSTH_data(:) - mean(PSTH_data(:))).^2);
RSquared_VAVA = 1 - rss_VAVA/TSS;
