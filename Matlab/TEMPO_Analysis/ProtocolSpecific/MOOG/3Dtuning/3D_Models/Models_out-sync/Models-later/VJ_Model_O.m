...
function r = VJ_Model_O(a,st_data)
...
% for i=1:size(r,1),
%     for j=1:size(r,2),
%         rr =a(1)*(a(12)*ele_azi_v(i,j)*vel_time + (1-a(12))*ele_azi_j(i,j)*jerk_time)+ a(2);
%         rr(find(rr<0))  = 0;
%         r(i,j,:) = rr;
%     end
% end
...
for i=1:size(r,1)
    for j=1:size(r,2)
        rr = a(12)*ele_azi_v(i,j)*vel_time + (1-a(12))*ele_azi_j(i,j)*jerk_time;
        r(i,j,:) = rr;
    end
end
