...
[rand_param(1,:),rand_rss(1),~,~,~,~,temp_jac] = lsqcurvefit('VJ_Model_O', ...
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_jac] = lsqcurvefit('VJ_Model_O', ...
...
respon = VJ_Model_O(modelFitPara_VJ,st_data);
modelFitRespon_VJ = respon;

modelFit_VJ.V = VJ_V_Com_O(modelFitPara_VJ([1:7,12]),st_data);
modelFit_VA.J = VJ_J_Com_O(modelFitPara_VJ([1:3,8:12,13]),st_data);
