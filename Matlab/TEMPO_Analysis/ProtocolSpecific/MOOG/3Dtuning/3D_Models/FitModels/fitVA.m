...
%     -2, ...         %20 a_DC
%     0,...           %21 w
%     -10];             %22 threshold for acceleration
% 
% UB = [4*A, ...      %1  A
%     300, ...        %2  R_0
%     mu+0.1, ...      %3  mu_t
...
% modelFit_VA.V = modelFitPara_VA(21).*VO_Model(modelFitPara_VA(1:12),st_data)+modelFitPara_VA(2);
% modelFit_VA.A = (1-modelFitPara_VA(21)).*AO_Model(modelFitPara_VA([1:4 13:20]),st_data)+modelFitPara_VA(2);
% modelFit_VA.V = modelFitPara_VA(21).*VO_Model(modelFitPara_VA(1:12),st_data);
% modelFit_VA.A = (1-modelFitPara_VA(21)).*AO_Model(modelFitPara_VA([1:4 13:20 end]),st_data);
modelFit_VA.V = VA_V_Com(modelFitPara_VA([1:12,21]),st_data);
modelFit_VA.A = VA_A_Com(modelFitPara_VA([1:4,13:20,21]),st_data);

% model fit spatial tuning
modelFit_VA_spatial.V = d_cos_tuning(modelFitPara_VA(5:12),st_data(1:13));
modelFit_VA_spatial.A = d_cos_tuning(modelFitPara_VA(13:20),st_data(1:13));
% modelFit_VA_spatial.V = cos_tuning(modelFitPara_VA(5:7),st_data(1:13));
% modelFit_VA_spatial.A = cos_tuning(modelFitPara_VA(13:15),st_data(1:13));
%% analysis
data_num = 26*nBins;
para_num = 10;
