...
rand_pac = zeros(reps+1, length(param), length(param));

[rand_param(1,:),rand_rss(1),~,~,~,~,temp_pac] = lsqcurvefit('VAP_Model', ...
    init_param(1,:), st_data, y_data, LB, UB, options);
rand_pac(1,:,:) = full(temp_pac)'*full(temp_pac);
...
for ii = 2:(reps + 1)
    ...
    [rand_param(ii,:),rand_rss(ii),~,~,~,~,temp_pac] = lsqcurvefit('VAP_Model', ...
        seed_param, st_data, y_data, LB, UB, options);
    rand_pac(ii,:,:) = full(temp_pac)'*full(temp_pac);
...
jac_VAP = rand_pac(min_inx,:,:);

% calculate the final model fitting values
respon = VAP_Model(modelFitPara_VAP,st_data);
modelFitRespon_VAP = respon;

modelFit_VAP.V = VAP_V_Com(modelFitPara_VAP([1:12,29,30]),st_data);
modelFit_VAP.A = VAP_A_Com(modelFitPara_VAP([1:4,13:20,29,30]),st_data);
modelFit_VAP.P = VAP_P_Com(modelFitPara_VAP([1:4,21:28,29,30]),st_data);

% model fit spatial tuning
modelFit_VAP_spatial.V = d_cos_tuning(modelFitPara_VAP(5:12),st_data(1:13));
modelFit_VAP_spatial.A = d_cos_tuning(modelFitPara_VAP(13:20),st_data(1:13));
modelFit_VAP_spatial.P = d_cos_tuning(modelFitPara_VAP(21:28),st_data(1:13));
% modelFit_VAP_spatial.V = cos_tuning(modelFitPara_VAP(5:7),st_data(1:13));
% modelFit_VAP_spatial.A = cos_tuning(modelFitPara_VAP(13:15),st_data(1:13));
% modelFit_VAP_spatial.P = cos_tuning(modelFitPara_VAP(21:23),st_data(1:13));
%% analysis
data_num = 26*nBins;
para_num = 14;
