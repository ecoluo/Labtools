...
function Popul_Analysis_3Dmodel_QQ(Monkey,Model_catg,models)
...
load(['Z:\Data\TEMPO\BATCH\',Monkey,'_3DTuning\',Model_catg,'\PSTH3DModel_OriData.mat']);
...
axes(h_subplot(ii));
hold on;
% temp = RSquared_T_vesti(:,ii);
% temp = temp(~isnan(temp));
[nelements, ncenters] = hist(RSquared_T_vesti,xR2);
...
plot(nanmedian(RSquared_T_vesti),max(nelements)*1.1,'kv', 'markerfacecolor','k');
text(nanmedian(RSquared_T_vesti)*1.1,max(nelements)*1.2,num2str(nanmedian(RSquared_T_vesti)));
...
axes(h_subplot(ii));
hold on;
[nelements, ncenters] = hist(RSquared_T_vis,xR2);
...
plot(nanmedian(RSquared_T_vis),max(nelements)*1.1,'kv', 'markerfacecolor','k');
text(nanmedian(RSquared_T_vis)*1.1,max(nelements)*1.2,num2str(nanmedian(RSquared_T_vis)));
% set(gca,'xtick',[0 500 1000 1500],'xticklabel',[],'xlim',[0 1600]);
% xlabel('Single-peaked');
title([models{ii},' model']);
axis on;
