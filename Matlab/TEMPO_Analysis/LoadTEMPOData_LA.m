...
%----------------------------------------------------------------------------------------------------------
function	[return_value, good_data, bad_data, newChans] = LoadTEMPOData_LA(PATH,FILE)
...
% for 3D_T_tuning, Azimuth = 0:45:315, Elevation = -45:45:45; 
...
if ff <= size(rescue_from_CED,1) % Cell that needs to be rescued
    ...
    % This adds the good data from kilosort2.
    fa = find(PATH == '\');
...
if (exist(sName,'file'))
    [good_data SortedChannel] = PackData_LA(good_data,sName);%get the Spike2 sorted data
    if SortedChannel >0
